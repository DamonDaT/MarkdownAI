...
> 在 LoRA 方法中，实际上是在原始预训练语言模型（PLM）旁增加一个附加的网络通路，这可以视作一种 **“外挂”** 结构。这个外挂结构的目的是通过两个矩阵 A 和 B 的相乘来模拟本征秩（Intrinsic rank）。A 矩阵用 **正态分布** 初始化，B 矩阵初始化为 **0**。
...
* 低估了权重矩阵的种类（选哪些权重矩阵，怎么选）和不同层对的微调效果影响。
